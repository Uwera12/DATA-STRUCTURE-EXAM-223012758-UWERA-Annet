#include <iostream> // This allows input and output functions (cout and cin operations).
#include <cstring> // cstring provides functions for working with c-style strings (like char arrays), such as copying text (strcpy) or comparing text (strcmp).
using namespace std; 
// Makes it easier to use standard C++ elements like cout and cin without writing std::
...
class ContactBase { // this code line create a parent or base class called ContactBase.
public:
 virtual void display() = 0; // A "pure virtual" function: every derived class MUST implement its own display() function.
...
class FriendContact : public ContactBase { //this code shows inheritance with a derived class or child class called FriendContact.
...
public: // a specifier
 ...
 cout << "[Friend] " << data.name << " - " << data.phone << endl; // this code will print a keyword called friend before name or phone number.
 }
 const char* getName() override { return data.name; } // Implementation of getName for FriendContact
};

class WorkContact : public ContactBase { //this code creates a derived or child class called WorkContact. 
 Contact data; // Contains the basic name and phone number
public:
 ...
 cout << "[Work] " << data.name << " - " << data.phone << endl; // this will print the keyword Work before name or phone number.
 }
